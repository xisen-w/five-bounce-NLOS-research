...
tic_Z = linspace(0,96e-4*256/2,512);
...
M = M/max(M(:));
M = min(M,ma);
scatter3(dep(M > thre),tic_X(M > thre),tic_Y(M > thre),M(M > thre)*20,M(M > thre), 'o','filled','MarkerFaceAlpha',1);
...
axis square;
set(gca,'color','k');
set(gcf,'position',[700,350,500,500]) 
end
% [M,dep] = max(data, [], 1);
